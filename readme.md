...
2.  Generate server code using wsimport command
   ...
   Wsimport 是 jdk 提供的 java 工具。您应该在您的 %JAVA_HOME%\bin 目录。在我的例子中，我在 C:\Program Files\Java\jdk1.8.0_241\bin 目录中找到它。
...
4.  Publish our service
   ...
   现在我们可以运行 *EmployeeServicePublisher* 来启动 web 服务。为了利用 CDI 特性，web 服务 可以作为 WAR 文件部署到应用服务器上，如 WildFly 或 GlassFish。
...
客户端可以使用服务端点接口中定义的方法。我们可以通过 wsimport 在客户端获得我们需要的代 码。
